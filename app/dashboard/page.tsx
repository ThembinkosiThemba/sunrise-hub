...
export default function DashboardPage() {
  ...
  const merchantServices = mockServices.filter(
    (s) => s.merchantId === currentMerchant.merchant_id
  );
  const merchantTransactions = mockTransactions.filter(
    (t) => t.merchantId === currentMerchant.merchant_id
  );
  const merchantPoints = mockPointsActivities.filter(
    (p) => p.merchantId === currentMerchant.merchant_id
  );
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 lg:gap-4">
              ...
              <Avatar className="h-8 w-8 lg:h-10 lg:w-10">
                <AvatarImage
                  src={currentMerchant.logo_url || "/placeholder.svg"}
                  alt={currentMerchant.display_name}
                />
                <AvatarFallback>
                  {currentMerchant.display_name[0]}
                </AvatarFallback>
              </Avatar>
              <Button
                variant="ghost"
      ...
      <div className="container mx-auto px-4 py-4 lg:py-8">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                <p className="text-sm lg:text-base text-charcoal/60">
                  Merchant ID: {currentMerchant.merchant_id}
                </p>
                <VerificationBadge
                  status={currentMerchant.verification_status}
                />
              </div>
            </div>
        ...
        <Tabs defaultValue="services" className="space-y-4 lg:space-y-6">
          <TabsList className="bg-white border border-charcoal/10 w-full grid grid-cols-2 lg:grid-cols-4 h-auto">
            <TabsTrigger
              value="services"
              className="data-[state=active]:bg-sunrise/10 data-[state=active]:text-sunrise text-xs lg:text-sm py-2.5 lg:py-2"
            >
              <Package className="h-4 w-4 mr-1.5 lg:mr-2" />
              <span>Services</span>
            </TabsTrigger>
            <TabsTrigger
              value="transactions"
              className="data-[state=active]:bg-sunrise/10 data-[state=active]:text-sunrise text-xs lg:text-sm py-2.5 lg:py-2"
            >
              <DollarSign className="h-4 w-4 mr-1.5 lg:mr-2" />
            ...
            <TabsTrigger
              ...
              className="data-[state=active]:bg-sunrise/10 data-[state=active]:text-sunrise text-xs lg:text-sm py-2.5 lg:py-2"
            >
              <BarChart3 className="h-4 w-4 mr-1.5 lg:mr-2" />
              <span>Analytics</span>
            </TabsTrigger>
            <TabsTrigger
              value="settings"
              className="data-[state=active]:bg-sunrise/10 data-[state=active]:text-sunrise text-xs lg:text-sm py-2.5 lg:py-2"
            >
              <Settings className="h-4 w-4 mr-1.5 lg:mr-2" />
              <span>Settings</span>
            </TabsTrigger>
          </TabsList>
            ...
            <Card>
              <CardHeader>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <CardTitle className="font-serif text-xl lg:text-2xl text-charcoal">
                      Your Services
                    </CardTitle>
                    <CardDescription className="text-xs lg:text-sm">
                      Manage your service listings ({merchantServices.length}
                      /24)
                    </CardDescription>
                  </div>
                  <Button
                    className="bg-sunrise hover:bg-sunrise-dark text-white w-full sm:w-auto"
                    size="sm"
                  >
                  ...
                  {merchantServices.map((service) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-sm lg:text-base text-charcoal mb-1">
                          {service.name}
                        </h3>
                        <p className="text-xs lg:text-sm text-charcoal/60 mb-2 line-clamp-2">
                          {service.description}
                        </p>
                    ...
                    >
                      <div className="flex items-center gap-3 lg:gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-sm lg:text-base text-charcoal">
                            {transaction.transactionType.replace("_", " ")}
                          </p>
                          <p className="text-xs lg:text-sm text-charcoal/60 truncate">
                            {transaction.transactionRef}
                          </p>
                        </div>
                      </div>
                      <div className="text-left sm:text-right">
                        <p className="font-bold text-sm lg:text-base text-charcoal">
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-sm text-charcoal/60 mb-1 block">
                          Display Name
                        </label>
                        <p className="font-medium text-charcoal">
                          {currentMerchant.display_name}
                        </p>
                      </div>
                      <div>
                        <label className="text-sm text-charcoal/60 mb-1 block">
