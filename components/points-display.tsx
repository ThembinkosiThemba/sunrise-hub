  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="flex items-center justify-between">
        <h2 className="font-serif text-3xl font-bold text-foreground">Sunrise Points</h2>
        <div className="flex items-center gap-3 rounded-full bg-primary/10 px-6 py-3">
          <Coins className="h-6 w-6 text-primary" />
          <span className="font-serif text-3xl font-bold text-primary">{points.total_points}</span>
        </div>
      </div>
            ...
            {points.recent_activities.map((activity) => (
              ...
              >
                <div className="flex-1">
                  <p className="font-medium text-foreground">{activity.activity_description}</p>
